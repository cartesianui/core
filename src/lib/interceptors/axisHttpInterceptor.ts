...
export class AxisHttpInterceptor implements HttpInterceptor {
    ...
    constructor(configuration: AxisHttpConfigurationService,
    ...
    protected addTenantIdHeader(headers: HttpHeaders): HttpHeaders {
        let cookieTenantIdValue = this._utilsService.getCookieValue(axis.multiTenancy.tenantIdCookieName);
        if (cookieTenantIdValue && headers && !headers.has(axis.multiTenancy.tenantIdCookieName)) {
            headers = headers.set(axis.multiTenancy.tenantIdCookieName, cookieTenantIdValue);
        }
